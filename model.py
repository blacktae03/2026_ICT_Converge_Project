...
max_voltages = np.array([5.0]) 

# 2. 안전장치 (Clipping): 노이즈로 튀는 값들을 강제로 0~Max 사이로 자름
# X_clipped = np.clip(Xt, 0.0, max_voltages)

# 3. 정규화 (Normalization)
X_norm = Xt / max_voltages
# 가상의 센서 데이터 (0V ~ 5V)
...
# 4. 학습
model.fit(Xt_poly, Yt, epochs=500, verbose=0)
...
# 실제 학습시킨 데이터 (빨간 점)
plt.scatter(Xt, Yt, color='red', s=150, label='Training Data', zorder=5)

# (3,3) 예측 지점 표시 (녹색 별)
test_val = np.array([[4.0]])
# 여기도 똑같이 전처리! (정규화 -> 다항식)
test_val_norm = test_val / max_voltages
test_val_poly = poly.transform(test_val_norm)

pred_val = model.predict(test_val_poly)
